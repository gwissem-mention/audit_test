	...
	function loadAndRender(view) {
		return function(settings) {
			...
			if (!Env.baseUrl) {
				...
				for (var i = 0; i < scripts.length; i++) {
					...
					if (/(^|\/)moxman\./.test(src)) {
						...
						Env.apiPageUrl = NodevoGestionnaireMediaBundle_MoxieManager.API_PHP_URL;
						break;
					}
				}
			}
			...
			function done() {
				...
				moxman.Env.apiPageUrl = NodevoGestionnaireMediaBundle_MoxieManager.API_PHP_URL;
			...
			if (!moxman.ui) {
				ResourceLoader.load({
					js: [
						...
						NodevoGestionnaireMediaBundle_MoxieManager.API_PHP_URL + "?action=language" + (settings.language ? '&code=' + settings.language : ''),
