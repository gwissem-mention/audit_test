...
$(function () {
    ...
    $('.panel-collapse').click(function() {
        $(this).find('a').children().toggleClass("fa-chevron-down fa-chevron-up");
        $(this).closest(".panel-heading").next().slideToggle({duration: 200});
        $(this).closest(".panel-heading").toggleClass('rounded-bottom');
        return false;
    });

    //hack de l'ouverture du menu
    if ( !$('#sidebar > li.active').hasClass('open') ) {
        $('#sidebar > li.active.hasChild a').click();
        $('#sidebar > li.active.hasChild .last.open').removeClass('open');    
    }
});
