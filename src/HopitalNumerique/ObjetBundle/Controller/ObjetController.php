...
class ObjetController extends Controller {
	...
	public function showAction($id) {
		...
		$outils = $this->get ( 'hopitalnumerique_autodiag.manager.outil' )->findBy ( array (
				'id' => $objet->getAutodiags () 
		) );
	...
	public function deleteMassAction($primaryKeys, $allPrimaryKeys) {
		...
		$objets = $this->get ( 'hopitalnumerique_objet.manager.objet' )->findBy ( array (
				'id' => $primaryKeys 
		) );
	...
	public function getPublicationsAction($articles) {
		if ($articles == 1) {
			$types = $this->get ( 'hopitalnumerique_reference.manager.reference' )->findBy ( array (
					'code' => 'CATEGORIE_OBJET' 
			) );
	...
	public function getPublicationDetailsForMenuAction() {
		...
		if (isset ( $publication [0] ) && isset ( $publication [1] )) {
			if ($publication [0] === 'PUBLICATION') {
				...
				$contenu = $this->get ( 'hopitalnumerique_objet.manager.contenu' )->findOneBy ( array (
						'id' => $publication [1] 
				) );
		...
		if ('POST' == $request->getMethod ()) {
			...
			if ($form->isValid ()) {
				...
				if (is_null ( $objet->isArticle () )) {
					...
					$domaines = $this->get ( 'hopitalnumerique_domaine.manager.domaine' )->findBy ( array (
							'id' => 1 
					) );
				...
				if ($notify === "1") {
					...
					if (count ( $objet->getDomaines () ) != 1) {
						if (in_array ( 1, $objet->getDomainesId () )) {
							$domaineUrl = $this->get ( 'hopitalnumerique_domaine.manager.domaine' )->findOneBy ( array (
									'id' => 1 
							) )->getUrl ();
						}
					} else {
					...
					foreach ( $consultations as $consultation ) {
						...
						$options = array (
								...
								'lienpublication' => '<a href="' . $domaineUrl . $this->generateUrl ( 'hopital_numerique_publication_publication_objet', array (
										'id' => $objet->getId (),
				...
				$this->get ( 'hopitalnumerique_glossaire.manager.glossaire' )->parsePublications ( array (
						$objet 
				) );
