...
class DemandeController extends \HopitalNumerique\InterventionBundle\Controller\DemandeController
{
     ...
     *
     * @return Response La vue de la visualisation d'une demande d'intervention
     */
    public function voirAction(InterventionDemande $id)
    {
        $interventionDemande = $id;

        $this->container->get('hopitalnumerique_intervention.service.demande.etat_type_derniere_demande')
     ...
     * @param null $filtre
     *
     * @return Response La vue de la liste de demandes d'intervention
     */
     ...
     *
     * @return Response La vue de la liste des demandes d'intervention
     */
     ...
     *
     * @param array $primaryKeys Les ID des demandes d'intervention sélectionnées par l'utilisateur
     *
    ...
    public function gridSupprimeMassAction(array $primaryKeys)
    {
        ...
        if ($this->get('nodevo_acl.manager.acl')->checkAuthorization(
            ...
            $interventionDemandes = $this->get('hopitalnumerique_intervention.manager.intervention_demande')->findBy(
                ['id' => $primaryKeys]
            );
            $this->get('hopitalnumerique_intervention.manager.intervention_demande')->delete($interventionDemandes);

            $this->get('session')->getFlashBag()->add('info', 'Suppression effectuée avec succès.');
        } else {
            $this->get('session')->getFlashBag()->add(
                'warning',
                'Vous ne possédez pas les droits nécessaires pour supprimer des interventions.'
            );
        }
    ...
    public function exportMassAction($primaryKeys, $allPrimaryKeys)
    {
        ...
        return $this->get('hopitalnumerique_intervention.manager.intervention_demande')->getExportCsv(
            $primaryKeys,
            $this->container->getParameter('kernel.charset')
        );
    }
    ...
    public function evaluationMassAction($primaryKeys, $allPrimaryKeys)
    {
        ...
        foreach ($interventionDemandes as $interventionDemande) {
            if (is_null($interventionDemande->getEvaluationEtat()) || $interventionDemande->getEvaluationEtat()->getId() !== InterventionEvaluationEtat::getInterventionEvaluationEtatEvalueId()) {
                $interventionDemande->setRemboursementEtat(
