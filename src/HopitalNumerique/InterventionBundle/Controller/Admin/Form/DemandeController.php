...
class DemandeController extends Controller
{
    ...
    public function nouveauAction()
    {
        ...
        $interventionDemandeFormulaire = $this->createForm('hopitalnumerique_interventionbundle_interventiondemande_admin', $this->interventionDemande, ['interventionDemande' => $this->interventionDemande]);
        if ($this->gereEnvoiFormulaireDemandeNouveau($interventionDemandeFormulaire)) {
            $do = $this->container->get('request')->request->get('do');

            return $this->redirect($do == 'save-close' ? $this->generateUrl('hopital_numerique_intervention_admin_liste') : $this->generateUrl('hopital_numerique_intervention_admin_demande_edit', ['id' => $this->interventionDemande->getId()]));
        }
    ...
    private function gereEnvoiFormulaireDemandeNouveau($interventionDemandeFormulaire)
    {
        if ($this->get('request')->isMethod('POST')) {
            ...
            if ($interventionDemandeFormulaire->isValid()) {
                ...
                $cmsi = $this->get('hopitalnumerique_user.manager.user')->getCmsi(['region' => $this->getUser()->getRegion(), 'enabled' => true]);
                if ($cmsi == null) {
                ...
                // Envoi des courriels
                $this->container->get('hopitalnumerique_intervention.manager.intervention_courriel')->envoiCourrielDemandeAcceptationAmbassadeur($this->interventionDemande->getAmbassadeur(), $this->generateUrl('hopital_numerique_intervention_demande_voir', ['id' => $this->interventionDemande->getId()], true));
    ...
    public function editAction(InterventionDemande $id)
    {
        ...
        $interventionDemandeFormulaire = $this->createForm('hopitalnumerique_interventionbundle_interventiondemande_edition_admin', $this->interventionDemande, ['interventionDemande' => $this->interventionDemande]);
...
//                    }
                ...
                if (!is_null($this->interventionDemande->getEvaluationEtat()) && $this->interventionDemande->getEvaluationEtat()->getId() === InterventionEvaluationEtat::getInterventionEvaluationEtatEvalueId()) {
                    $this->interventionDemande->setRemboursementEtat($this->get('hopitalnumerique_reference.manager.reference')->findOneBy(['id' => 5]));
    ...
    private function gereEnvoiMailChangementEtat(InterventionDemande $interventionDemande)
    {
        ...
        if ($etatId == InterventionEtat::getInterventionEtatRefusCmsiId()) {
            ...
            $this->container->get('hopitalnumerique_intervention.manager.intervention_courriel')->envoiCourrielDemandeAcceptationAmbassadeur(
                ...
                $this->generateUrl('hopital_numerique_intervention_demande_voir', ['id' => $interventionDemande->getId()], true)
            );
