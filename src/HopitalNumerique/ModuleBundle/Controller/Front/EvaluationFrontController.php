...
class EvaluationFrontController extends Controller
{
    ...
    private function renderForm(Session $session, $readOnly = false)
    {
        ...
        $questionnaire = $this->get('hopitalnumerique_questionnaire.manager.questionnaire')->findOneBy(['id' => $idQuestionnaireModuleEvaluation]);
        ...
        if ($form->handleRequest($request)->isValid()) {
            ...
            if ($this->container->get('security.context')->isGranted('ROLE_AMBASSADEUR_7')) {
                ...
                $connaissancesMetiers = [];

                foreach ($connaissancesMetierSession as $connaissanceMetierSession) {
                    $connaissanceMetier = $this->get('hopitalnumerique_user.manager.connaissance_ambassadeur')->findOneBy(['user' => $user, 'domaine' => $connaissanceMetierSession]);
                    //Si il a deja cette connaissance, on ne le rajoute pas
                    if (!is_null($connaissanceMetier) && !is_null($connaissanceMetier->getConnaissance())) {
                        ...
                        $connaissanceMetier->setConnaissance($this->get('hopitalnumerique_reference.manager.reference')->findBy(['code' => 'CONNAISSANCES_AMBASSADEUR'], ['order' => 'ASC'])[1]);
            ...
            return $this->redirect($this->generateUrl('hopitalnumerique_module_evaluation_view_front', ['id' => $session->getId()]));
        }

        return $this->render('HopitalNumeriqueModuleBundle:Front/Evaluation:form.html.twig', [
