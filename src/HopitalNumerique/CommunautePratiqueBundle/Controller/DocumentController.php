...
class DocumentController extends \Symfony\Bundle\FrameworkBundle\Controller\Controller
{
    ...
    public function listByGroupeAction(Groupe $groupe)
    {
        ...
        return $this->render('HopitalNumeriqueCommunautePratiqueBundle:Document:listByGroupe.html.twig', [
            'groupe' => $groupe,
            'documents' => $this->container->get('hopitalnumerique_communautepratique.manager.document')->findBy(['groupe' => $groupe, 'user' => $this->getUser()]),
    ...
    private function saveUploadedDocument(UploadedFile $uploadedFile, Groupe $groupe)
    {
        ...
        if ($documentFichier->getSize() > Document::MAX_SIZE * 1024 * 1024) {
            ...
            return [$uploadedFile->getClientOriginalName() => false]; // Échec
        }
    }
